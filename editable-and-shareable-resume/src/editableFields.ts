...
function generateEditableContent(
  field: EditableField,
  fieldType: FieldType
): string {
  let content = ""; // Initialize content here to avoid reference errors
  ...
  switch (fieldType) {
    ...
    default:
      ...
      return ""; // Return an empty string if fieldType is not recognized
  }

  return content; // Make sure to return content
}

const saveEdit = (field: EditableField, fieldType: FieldType): void => {
  // Generate the saved content using helper function
  const newContent = generateSavedContent(field, fieldType);

  // Update the field's HTML and remove editing class
